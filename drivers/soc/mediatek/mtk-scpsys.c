...
/*
 * MT8695 power domain support
 */

static const struct scp_domain_data scp_domain_data_mt8695[] = {
	[MT8695_POWER_DOMAIN_MM] = {
		.name = "mm",
		.sta_mask = BIT(11),
		.ctl_offs = 0x026c,
		.sram_pdn_bits = GENMASK(11, 8),
	...
	[MT8695_POWER_DOMAIN_VENC] = {
		.name = "venc",
		.sta_mask = BIT(13),
		.ctl_offs = 0x0274,
		.sram_pdn_bits = GENMASK(11, 8),
		.sram_pdn_ack_bits = GENMASK(14, 12),
		.clk_id = {CLK_MM, CLK_VENC, CLK_DEMUX},
		.active_wakeup = true,
	},
	[MT8695_POWER_DOMAIN_ETH] = {
		.name = "eth",
		.sta_mask = BIT(22),
		.ctl_offs = 0x02bc,
		.sram_pdn_bits = GENMASK(11, 8),
...
#define NUM_DOMAINS_MT8695	ARRAY_SIZE(scp_domain_data_mt8695)

static int __init scpsys_probe_mt8695(struct platform_device *pdev)
{
	struct scp *scp;
	struct genpd_onecell_data *pd_data;
	int ret;

	scp = init_scp(pdev, scp_domain_data_mt8695, NUM_DOMAINS_MT8695);
	if (IS_ERR(scp))
		return PTR_ERR(scp);

	mtk_register_power_domains(pdev, scp, NUM_DOMAINS_MT8695);

	pd_data = &scp->pd_data;

	ret = pm_genpd_add_subdomain(pd_data->domains[MT8695_POWER_DOMAIN_MM],
		pd_data->domains[MT8695_POWER_DOMAIN_VDEC]);
	if (ret && IS_ENABLED(CONFIG_PM))
		dev_err(&pdev->dev, "Failed to add subdomain vdec: %d\n", ret);

	ret = pm_genpd_add_subdomain(pd_data->domains[MT8695_POWER_DOMAIN_MM],
		pd_data->domains[MT8695_POWER_DOMAIN_DIS_M]);
	if (ret && IS_ENABLED(CONFIG_PM))
		dev_err(&pdev->dev, "Failed to add subdomain dis_m: %d\n", ret);

	ret = pm_genpd_add_subdomain(pd_data->domains[MT8695_POWER_DOMAIN_DIS_M],
		pd_data->domains[MT8695_POWER_DOMAIN_DIS_S]);
	if (ret && IS_ENABLED(CONFIG_PM))
		dev_err(&pdev->dev, "Failed to add subdomain dis_s: %d\n", ret);

	ret = pm_genpd_add_subdomain(pd_data->domains[MT8695_POWER_DOMAIN_DIS_M],
		pd_data->domains[MT8695_POWER_DOMAIN_DIS_C]);
	if (ret && IS_ENABLED(CONFIG_PM))
		dev_err(&pdev->dev, "Failed to add subdomain dis_c: %d\n", ret);

	ret = pm_genpd_add_subdomain(pd_data->domains[MT8695_POWER_DOMAIN_MM],
		pd_data->domains[MT8695_POWER_DOMAIN_DOLBY]);
	if (ret && IS_ENABLED(CONFIG_PM))
		dev_err(&pdev->dev, "Failed to add subdomain dolby: %d\n", ret);

	ret = pm_genpd_add_subdomain(pd_data->domains[MT8695_POWER_DOMAIN_MM],
		pd_data->domains[MT8695_POWER_DOMAIN_VENC]);
	if (ret && IS_ENABLED(CONFIG_PM))
		dev_err(&pdev->dev, "Failed to add subdomain venc: %d\n", ret);
...
static const struct of_device_id of_scpsys_match_tbl[] = {
	{
		...
		.compatible = "mediatek,mt8695-scpsys",
		.data = scpsys_probe_mt8695,
	}, {
		/* sentinel */
	}
};
