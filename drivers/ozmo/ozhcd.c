			...
			u8 length, u16 offset, u16 total_size)
{
	...
	if (status == 0) {
		...
		if (length <= copy_len)
			copy_len = length;
		if ((urb->transfer_buffer+offset) && desc && (copy_len > 0))
				...
				int data_len)
{
	...
	if(data_len > ep->buffer_size)
		return -1;
	if (copy_len > data_len)
		copy_len = data_len;
	if (data && (&ep->buffer[ep->in_ix]) && (copy_len > 0))
		memcpy(&ep->buffer[ep->in_ix], data, copy_len);

	if (copy_len < data_len) {
		if (ep->buffer && (data+copy_len))
				...
				u8 frame, const u8 *data, int data_len)
{
	...
	if (copy_len > data_len)
		copy_len = data_len;
	if ((&ep->buffer[ep->in_ix]) && data  && (copy_len > 0))
		memcpy(&ep->buffer[ep->in_ix], data, copy_len);
