...
TRACE_EVENT(urb_in,
	...
	TP_fast_assign(
		...
		if (__entry->endpoint == 0x80) {
			__entry->inc_length = 8;
			memcpy(__entry->buffer, oz_urb->setup_packet, 8);
		} else if (__entry->endpoint == 0x00) {
			__entry->inc_length += 8;
			if (__entry->inc_length >= MAX_URB_LEN)
				__entry->inc_length = MAX_URB_LEN;
			memcpy(__entry->buffer, oz_urb->setup_packet, 8);
			memcpy(&__entry->buffer[8],
				oz_urb->transfer_buffer,
				__entry->inc_length - 8);
		} else {
	...
	__entry->buffer[14], __entry->buffer[15], __entry->buffer[16],
	__entry->buffer[17], __entry->buffer[18], __entry->buffer[19],
	__entry->buffer[20], __entry->buffer[21], __entry->buffer[22],
	__entry->buffer[23], __entry->buffer[24], __entry->buffer[25],
	__entry->buffer[26], __entry->buffer[27], __entry->buffer[28],
	__entry->buffer[29], __entry->buffer[30], __entry->buffer[31])
