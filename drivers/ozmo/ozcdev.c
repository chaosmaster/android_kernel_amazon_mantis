		...
		loff_t *fpos)
{
	...
	if (is_tftp) {
		...
		copy_len = n;
		if (copy_len > count) {
			oz_trace_msg(M, "ENOMEM buf=%d pkt=%d\n", (int)count, n);
			copy_len = count;
		}

		/* copy n bytes of datagram to user bufer */
		if ((ix + n) < OZ_RD_BUF_SZ) {
			if (copy_to_user(buf, &ctx->rd_buf[ix], copy_len)) {
				count = 0;
				goto out1;
			}
			spin_lock_bh(&ctx->rd_lock);
			...
			if (copy_len > b) {
				...
				if (copy_to_user(&buf[b], ctx->rd_buf, copy_len - b)) {
					count = 0;
					goto out1;
				}
			} else {
				/* All data we need is in the first half. */
				if (copy_to_user(buf, &ctx->rd_buf[ix], copy_len)) {
					count = 0;
					goto out1;
				}
		...
		loff_t *fpos)
{
	...
	if (count > sizeof(ei->data) - sizeof(*elt) - sizeof(*app_hdr)) {
		oz_trace_msg(O, "WRITE O %08X %04X\n",
				 (unsigned int)((uintptr_t)filp), -EINVAL);
		return -EINVAL;
	}
