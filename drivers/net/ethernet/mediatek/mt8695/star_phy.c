...
static struct eth_phy_ops rtl8211fd_phy_ops = {
	.phy_id = PHYID2_RTL8211FD,
};
...
int star_detect_phyid(star_dev *dev)
{
	...
	for (addr = 0; addr < 32; addr++) {
		...
		if (reg2 == PHYID2_SMSC8710A) {
			...
			dev->phy_ops->addr = addr;
			break;
		} else if ((reg2 == PHYID2_RTL8211FD) &&
			   (addr != 0)) {
			STAR_MSG(STAR_DBG, "Ethernet: RTL8211FD PHY\n\r");
			dev->phy_ops = &rtl8211fd_phy_ops;
			dev->phy_ops->addr = addr;
			break;
		}
	}
