...
static int t_power_u1u2(struct xhci_hcd_mtk *mtk, int argc, char **argv)
{
	...
	if (argc > 1 && kstrtol(argv[1], 10, (long *)&port_id))
		xhci_err(xhci, "mu3h %s get port-id failed\n", __func__);

	if (argc > 2 && kstrtol(argv[2], 10, (long *)&u_num))
		xhci_err(xhci, "mu3h %s get u_num failed\n", __func__);

	if (argc > 3 && kstrtol(argv[3], 10, (long *)&u1_val))
		xhci_err(xhci, "mu3h %s get u1_val failed\n", __func__);

	if (argc > 4 && kstrtol(argv[4], 10, (long *)&u2_val))
...
#define REGS_IPPC_OFFSET 0x10700
#define REGS_PHY_OFFSET  0x290000
...
static void __iomem *get_reg_base_limit(struct xhci_hcd_mtk *mtk,  const char *buf, u32 *limit)
{
	...
	case 'p':
		...
		len = REGS_LIMIT_PHYS;
		xhci_info(xhci, "func:%s,line%d ,REGS_PHY_OFFSET ==0x%x\n", __func__, __LINE__, REGS_PHY_OFFSET);
...
static void ssusb_read_regs(struct xhci_hcd_mtk *mtk, const char *buf)
{
	...
	xhci_info(xhci, "params-%d (base: %p,offset: %#x, len: %#x)\n", param, base, offset, len);
	if (param == 1)
...
static int  ssusb_remap_ip_regs(struct device *dev)
{
	...
	mtk->ip_base = ioremap(hcd->rsrc_start, 0x300000);
	if (!mtk->ip_base)
