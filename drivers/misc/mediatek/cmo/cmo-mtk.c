...
void smp_inner_dcache_flush_all(void)
{
	...
	if (in_interrupt()) {
		pr_notice("Cannot invoke smp_inner_dcache_flush_all() in interrupt/softirq context\n");
		return;
	}
		...
		case FLUSHING:
			...
			for (j = 0; j < npages_this_entry; j++) {
				...
				if (unlikely(IS_ERR_OR_NULL((void *) start))) {
					pr_notice("[smp cache flush] cannot do cache sync: ret=%lu\n", start);
				...
				if (likely(ion_sync_kernel_func))
					...
					pr_notice("[smp cache flush] ion_sync_kernel_func is NULL\n");
...
int mt_smp_cache_flush(struct sg_table *table, unsigned int sync_type, int npages)
{
	...
	if (in_interrupt()) {
		pr_notice("Cannot invoke mt_smp_cache_flush() in interrupt/softirq context\n");
		return -EFAULT;
	}
