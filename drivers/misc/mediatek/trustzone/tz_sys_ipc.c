...
#if defined(__arm__) && defined(CONFIG_ARM_PSCI)
	/* For 32bit kernel/64bit tee case,
	* 64bit tee fill REE params with "unsigned long"
	* it needs map to "unsigned long long" in 32bit kernel
	*/
	unsigned long long *in;

	in = (unsigned long long *)&param[0];
	sema = (struct semaphore *)(long)in[0];
	jiffies = (long)in[1];
#else
	unsigned long *in;
