		       ...
		       struct pt_regs *regs)
{
    ...
    pr_err("%s: addr 0x%lx, fsr : %d\n", __func__,  addr, fsr);

	status = readl(IOMEM(BUS_DBG_CON));

	pr_err("%s: show status %x \n", __func__, status);
...
static int systracker_platform_test_init(void)
{
	...
	/*p1 = ioremap(0x10006214, 0x4);*/
	p1 = ioremap(0x10006290, 0x1000);
	/* use mmsys reg base for our test */
	/*mm_area1 = ioremap(0x14000000, 0x4);*/
	/*mm_area1 = ioremap(0x13050000, 0x4);*/
	mm_area1 = ioremap(0x13FFE000, 0x1000);
...
{
	pr_err("[LY]enter systracker_platform_read_timeout_test\n");
	writel(readl(p1) & (0xfffffffe), p1);
	/*writel(readl(p1) | (0x1 << 6), p1);*/
	readl(mm_area1);
	/*writel(readl(p1) & ~(0x1 << 6), p1);*/
	writel(readl(p1) | (0x00000001), p1);
...
{
	pr_err("[LY]before:0x10006290:0x%x\n", readl(p1));
	writel(readl(p1) & (0xfffffffe), p1);
	pr_err("[LY]after:0x10006290:0x%x\n", readl(p1));
	writel(0x0, mm_area1);
	writel(readl(p1) | (0x00000001), p1);
}
...
MODULE_DESCRIPTION("system tracker test suite v1.0");
