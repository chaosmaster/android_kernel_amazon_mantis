...
static void mtk_ir_irq_function(struct work_struct *work)
{
	...
	if ((scancode != BTN_INVALID_KEY) && (scancode != BTN_NONE)) {
		if (likely(rcdev != NULL)) {
...
#ifdef CONFIG_MTK_IRRX_MOUSE_DRAG
			else if ((dev_mode == MTK_IR_AS_MOUSE) && (scancode == p_mousecode->dragswitch)) {
				MTK_IR_LOG(" Drag mouse switch mode code 0x%08x\n", scancode);
				if (!(rcdev->keypressed)) { /* Switch between normal mouse and drag mouse */
			...
			} else if ((dev_mode == MTK_IR_AS_MOUSE) && (mouse_mode == MTK_IR_AS_DRAG_MOUSE)) {
				if (mtk_ir_drag_mouse_proc_key(scancode, cxt))
					MTK_IR_LOG(" process drag mouse key\n");
				else {	/* this key is not drag mouse key ,so as ir key process */
