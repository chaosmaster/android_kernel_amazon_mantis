...
static int __init create_sysfs(void)
{
	...
	if (bm == META_BOOT || bm == ADVMETA_BOOT || bm == ATE_FACTORY_BOOT || bm == FACTORY_BOOT) {
		...
		if (of_chosen) {
			tags =
			  (struct boot_tag_meta_com *)of_get_property(of_chosen, "atag,meta", NULL);
			if (tags) {
				g_meta_com_type = tags->meta_com_type;
				g_meta_com_id = tags->meta_com_id;
				g_meta_uart_port = tags->meta_uart_port;
				pr_debug
				    ("g_meta_com_type=%d, g_meta_com_id=%d, g_meta_uart_port=%d\n",
				    g_meta_com_type, g_meta_com_id, g_meta_uart_port);
			} else
				pr_warn("No atag,meta found !\n");
		} else
			pr_warn("of_chosen is NULL !\n");
#endif
